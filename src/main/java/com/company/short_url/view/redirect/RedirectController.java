...
public class RedirectController {
    ...
    private ShortUrl getShortUrl(String shortPath) {
        ShortUrl shortUrl = dataManager.load(ShortUrl.class).query("e.shortUrl=?1",
                shortPath).hint(PersistenceHints.SOFT_DELETION, false).one();
        return shortUrl;
    }
    ...
    private String selectFullUrl(ShortUrl shortUrl, String shortPath) {
        ...
        if (shortUrl.getRedirectLimit() != null) {
            ...
            resultShortUrl = dataManager.load(ShortUrl.class).query("e.shortUrl=?1 " +
                            "and e.deletedDate is null and e.accessCount < ?2",
                    shortPath,
                    redirectLimit).one();
        } else {
